...
def remap_item(train_data, test_data):
    """Count users/items and build per-user interaction sets."""
...
def read_triplets(file_name):
    """Read KG triplets from file and compute entity/relation counts."""
...
def build_graph(triplets):
    """Build an undirected NetworkX graph from KG triplets."""
    ckg_graph = nx.Graph()
    rd = defaultdict(list)
...
def build_sparse_item2entity_graph(relation_dict):
    """Build a sparse item-entity adjacency matrix for critiquing keyphrase extraction."""

    def counting_interact(adj_mat_list):
        """Sum all relation-specific adjacency matrices."""
    ...
    def counting_nhop_interact(kg_mat):
        """Compute n-hop item-entity adjacency by matrix power."""
...
def load_data(model_args):
    """Load train/test interactions and build user dicts (without KG)."""
    global args
    args = model_args
...
def load_kg(model_args):
    """Load KG triplets and build item-entity mapping structures for critiquing."""
    """entity2item & item2entity"""
    entity2item = defaultdict(list)
    item2entity = defaultdict(list)

    entity_neighbor_1_hop = defaultdict(list)
    ...
    # Build bidirectional item<->entity mappings
    ...
    # Build 1-hop neighbor sets for each entity (used for Jaccard similarity)
    kg_nodes = list(nx.nodes(kg_graph))
    for Fn in tqdm(kg_nodes, ascii=True):
        nei1_li=[]
        for n1 in list(kg_graph.neighbors(Fn)):
            nei1_li.append(n1)
        entity_neighbor_1_hop[Fn]=set(nei1_li)
    ...
    return kg_graph, entity2item, item2entity, item_entity_mat, n_params, entity_neighbor_1_hop
