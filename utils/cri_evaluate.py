...
def ranklist_by_heapq(user_pos_test, test_items, rating, Ks):
    """Rank items by rating scores using a max-heap, return top-K relevance list."""
    item_score = {}
    for i in test_items:
...
def get_auc(item_score, user_pos_test):
    """Compute AUC score for a single user."""
...
def ranklist_by_sorted(user_pos_test, test_items, rating, Ks):
    """Rank items and also compute AUC (slower than heapq version)."""
    item_score = {}
    for i in test_items:
        item_score[i] = rating[i]
...
def get_performance(user_pos_test, r, auc, Ks):
    """Compute precision, recall, NDCG, and hit ratio at various K values."""
...
def test_one_user(x):
    """Evaluate recommendation metrics for a single user."""
    rating = x[0]
    u = x[1]
    try:
...
def test(model, user_dict, n_params):
    """Full evaluation: compute recall, NDCG, precision, hit_ratio over all test users."""
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            # batch-item test: process items in mini-batches to save memory
