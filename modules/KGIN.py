...
class Aggregator(nn.Module):
    ...
    Aggregates neighbor information from KG and user-item interactions.
    """
...
class GraphConv(nn.Module):
    """
    Graph Convolutional Network with disentangled representations.
    Supports multiple independence modeling strategies: MI, distance, cosine.
    """
    def __init__(self, channel, n_hops, n_users,
        ...
        # Relation-specific transformation weights
        weight = initializer(torch.empty(n_relations - 1, channel))
        self.weight = nn.Parameter(weight)

        # Disentangle attention weights: [n_factors, n_relations-1]
    ...
    def _edge_sampling(self, edge_index, edge_type, rate=0.5):
        """Randomly drop edges for node-level dropout."""
        n_edges = edge_index.shape[1]
        random_indices = np.random.choice(n_edges, size=int(n_edges * rate), replace=False)
        return edge_index[:, random_indices], edge_type[random_indices]

    def _sparse_dropout(self, x, rate=0.5):
        """Dropout on sparse tensor values."""
        noise_shape = x._nnz()
    ...
    def _cul_cor(self):
        """Compute correlation/dependence loss between disentangled factors.
        Supports: mutual information, distance correlation, cosine similarity.
        """
        def CosineSimilarity(tensor_1, tensor_2):
            # Squared cosine similarity (always non-negative)
        ...
        def MutualInformation():
            # Mutual information minimization between disentangled factors
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
        ...
        for i in range(len(self.convs)):
            ...
            """residual connection: accumulate embeddings from each hop"""
...
class Recommender(nn.Module):
    """KGIN recommender model: Knowledge Graph-based disentangled recommendation."""
    ...
    def _init_weight(self):
        initializer = nn.init.xavier_uniform_
        # Unified embedding for users + entities
        self.all_embed = initializer(torch.empty(self.n_nodes, self.emb_size))
        # Disentangled latent factor embeddings
        self.latent_emb = initializer(torch.empty(self.n_factors, self.emb_size))

        # User-item interaction sparse matrix
    ...
    def _get_edges(self, graph):
        """Extract edge index and edge type from the KG graph."""
    ...
    def forward(self, batch=None):
        """Forward pass: compute BPR loss for a batch of (user, pos, neg) triples."""
        user = batch['users']
    ...
    def generate(self):
        """Generate final embeddings without dropout (for evaluation)."""
        user_emb = self.all_embed[:self.n_users, :]
        item_emb = self.all_embed[self.n_users:, :]
    ...
    def rating(self, u_g_embeddings, i_g_embeddings):
        """Compute user-item rating scores via dot product."""
        return torch.matmul(u_g_embeddings, i_g_embeddings.t())

    def create_bpr_loss(self, users, pos_items, neg_items, cor):
        """BPR loss with L2 regularization and disentangle correlation loss."""
