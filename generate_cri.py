...
def get_cri_entities(user_pos_set, need_ranking_items, rating):
    """Identify critique keyphrases for a user based on rating differences."""

    # Find top-ranked items that the user has NOT interacted with
    need_cri_items = get_highest_item(user_pos_set, need_ranking_items, rating)

    # Extract keyphrases where the user's positive items are weak
    need_cri_key = get_cri_keyphrase(user_pos_set, need_cri_items)

    return need_cri_key

def get_highest_item(user_pos_set, need_ranking_items, rating):
    """Select top-10 ranked items, then keep at most 5 that the user hasn't seen."""
    item_score = {}
    for i in need_ranking_items:
        item_score[i] = rating[i]

    top_10_items = heapq.nlargest(10, item_score, key = item_score.get)

    cri_items = []

    top_rank_items = top_10_items
...
def get_cri_keyphrase(user_pos_set, need_cri_items):
    """Find keyphrases where the recommended items are strong but the user's positive items are weak."""

    # Aggregate keyphrase distributions for positive items and critique items
    pos_keyphrase = get_item_keyphrase(user_pos_set)
    cri_items_keyphrase = get_item_keyphrase(need_cri_items)

    # Keyphrases where critique items surpass the user's positive items
    keyphrase_diff = pos_keyphrase - cri_items_keyphrase
    keyphrase_cri_index = np.where(keyphrase_diff < 0)[0]
    # Filter to only keyphrase entities (index >= n_items)
    only_key_cri_index = keyphrase_cri_index[keyphrase_cri_index >= n_items]
    only_key_cri_index_re = only_key_cri_index.tolist()

    # Limit to top-K most different keyphrases
...
def get_item_keyphrase(items):
    """Sum up the KG adjacency rows for the given items to get a keyphrase distribution."""
...
def test_one_user(x):
    """Generate critique keyphrases for a single user."""
...
def generate_cri_key(model, user_dict, n_params, kg_matrix, args_):
    """Generate critique keyphrases for all test users using the trained model."""
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            # batch-item test: compute ratings in item mini-batches to save GPU memory
            ...
            # all-item test: compute all ratings at once
